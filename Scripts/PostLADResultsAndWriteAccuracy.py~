...
import GenerateLETORFormDiscriminantTrainingData as GLFD
import DataFile
...
if __name__ == '__main__':
    ...
    options = ArgLib.parse(['init', 'ppmstd', 'dtadir', 'lads', 'sequest', 'config', 'model', 'output', 'symbolmap'], optArgs=[{'opts': ('-D', '--discmodel'), 'attrs': {'type': 'string', 'dest': 'discmodel', 'help': 'Model used to calculate discriminant score'}}])
...
#    rankModel = loadRankingModel(options.discmodel)
    ...
    for seqEntry in LADSSeqInfo:
        ...
        try:
            scanScoreDict = getScanScoreDictLinearDiscriminator(LADSSeqInfo, seqEntry, scanFDict, linearDiscVec, pairConfigurations['silac_light_heavy'], PNet)
        except ValueError:
            continue
...
#        scanScoreDict = getScanScoreDictClusterNormScore(LADSSeqInfo, seqEntry)
        ...
        for i, scan in enumerate(heavyScans):
            ...
            heavySeq = An.preprocessSequence(scanScoreDict[scan]['Seq'][0], heavySeqMaps['silac_light_heavy'], replaceExistingTerminalMods=True, ambigEdges=scanScoreDict[scan]['Seq'][1])
