...
# Does not take into account enzyme specificity prior for in-graph scoring, will consider in post scoring
# Implement decoy model (for noise scorer)?
...
def getScoreFromPRMs(spec, prms, deNovoTerm, addTerminalNodes=True):
    if addTerminalNodes:
        if deNovoTerm == 'N':
            prms = prms[1:]
    ...
    score = 0
    for prm in prms:
        score += spec.getNodeScore(**{'prm': prm, 'formAA': 'X', 'lattAA': 'X'})

    return score
    

def getDeNovoTerm(tag, lenPept):
    if tag[0][0] == 0:
        return 'N'
...
# Can make this function more efficient by hashing locations array
def getPossModLocationIndexes(modData, refSubSeq, term, unimodDict):

    if modData[0] == 'Undefined Mass Shift':
        return range(len(refSubSeq))

    indexes = set()
    for location in unimodDict[modData[0]]['locations']:
        if location[0] == term:
            indexes.add(0 if term == 'N-term' else len(refSubSeq) - 1)

        elif location[1] == term:
            if term == 'N-term' and refSubSeq[0] == location[0]:
                indexes.add(0)
            elif term == 'C-term' and refSubSeq[-1] == location[0]:
                indexes.add(len(refSubSeq) - 1)

        elif location[1] == 'Anywhere':
            for i in range(len(refSubSeq)):
                if refSubSeq[i] == location[0]:
                    indexes.add(i)

    return indexes

def getModPeptides(modData, refSubSeq, term, unimodDict):

    indexes = getPossModLocationIndexes(modData, refSubSeq, term, unimodDict)
    
    modPepts = []
    for index in indexes:
        modPepts += [( refSubSeq[:index] + 'X' + refSubSeq[index+1:], [(0, Constants.aminoacids[refSubSeq[index]][2] + modData[2])] )]
...
def getBestAlignment(tagGraph, dbPept, maxScore, maxScoringTag):
    ...
    while True:
        for node in tagGraph.predecessors(currentNode):
            ...
            if abs(currentScore - tagGraph.edge[node][currentNode]['edgeScore'] - tagGraph.node[node]['score']) < 0.0001:
...
def resolveModification(hUnimodDict, unimodDict, deNovoMass, refMass, deNovoSeq, unmodSeq, paramsDict, term = None, nModSymbol=None, epsilon=0.02):
    ...
    # Again, only until better scoring
    refMass = refMass + (Constants.mods[nModSymbol] if nModSymbol else 0)
            ...
            if abs(possMods[mod]) < epsilon:
                ...
                for location in locations:
                    if isValidModLocation(unmodSeq, term, location):
                        # deNovoMass - refMass is the 'mass to add' to ref sequence for efficient calculation of PRM ladder
                        matchedMods += [(mod, possMods[mod], massToAdd, deNovoMass, deNovoSeq, unmodSeq)]
                        break

    if len(matchedMods) > 0:
        return matchedMods
    else:
        return (('Undefined Mass Shift', None, massToAdd, deNovoMass, deNovoSeq, unmodSeq),)
...
def resolveInterval(refMass, deNovoMass, refSubSeq, deNovoSubSeq, hUnimodDict, unimodDict, paramsDict, term=None, nModSymbol=None, epsilon=0.02):
    if not refSubSeq and not deNovoSubSeq:
        ...
        return tuple(resolveModification(hUnimodDict, unimodDict, deNovoMass, refMass, deNovoSubSeq, refSubSeq, paramsDict, term = term, nModSymbol=nModSymbol, epsilon=epsilon))
...
def getExtendedSequence(seqDict, reference, nTermMass=0, cTermMass=0):
    ...
    if nTermMass > 0:
        ...
        for aa in protSeq[:reference[1]-1][::-1]:
        ...
        if scanData['Unmod Peptide'] == subjSequence:
            ...
            for reference in refList:
                ...
                if int(entry['Query Start']) > 1:
                    ntermMass = Constants.getMass(scanData['Unmod Peptide'][:int(entry['Query Start'])-1]) + 250
                if int(entry['Query End']) < len(scanData['Unmod Peptide']):
                    ctermMass = Constants.getMass(scanData['Unmod Peptide'][int(entry['Query End']):]) + 250
                    
                extendedSequence = getExtendedSequence(seqDict, reference, nTermMass, cTermMass)
                
                if extendedSequence not in modLists:

                    dbPept, modList, maxScore = alignDeNovoToDBSequence(scanData['Peptide'], scanData['Unmod Peptide'], extendedSequence, spec, hashedUnimodDict, unimodDict, paramsDict, deNovoAmbigEdges = ambigEdges, tagLength=2, isobaricPenalty=-0.5, defModPenalty=-1, inDelPenalty=-3, undefModPenalty=-3, defaultScore=0)
