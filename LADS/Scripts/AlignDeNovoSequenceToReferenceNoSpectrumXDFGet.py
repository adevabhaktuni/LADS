...
def getExtendedSequence(fastaFile, reference, nTermMass=0, cTermMass=0):
    protSeq = getSequence(fastaFile, reference[0])
    ...
    return removeNoncanonicalAminoAcids(baseSeq)
    ...
    for entry in DataFile.getScanInfo(options.comp, delimiter='\t'):
        ...
        if scanData['Unmod Peptide'] == subjSequence:
            ...
            for reference in refList:
                ...
                extendedSequence = getExtendedSequence(options.fasta, reference, nTermMass, cTermMass)
                print reference, extendedSequence
            ...
            sortedMods = sorted(modLists.items(), key=lambda k: -k[1][2])

            maxScore = sortedMods[0][1][2]
            if maxScore == None:
                continue
