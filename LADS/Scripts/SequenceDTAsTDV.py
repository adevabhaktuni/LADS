...
sys.path.insert(1, os.path.abspath(os.pardir + "/libsvm-3.12/python"))
import glob
import time
import pickle
import numpy as np
...
if __name__ == '__main__' :
    ...
    clusterSVMModel = svmutil.svm_load_model(os.pardir + paramsDict['Cluster Configuration']['model'])
    clusterSVMRanges = svmutil.load_ranges(os.pardir + os.path.splitext((paramsDict['Cluster Configuration']['model']))[0] + '.range')
...
#    samePeptideClusters = An.getSamePeptideClusters(precMassClusters, scanFDict, clusterSVMModel, clusterSVMRanges, ppmSTD=options.ppmstd, cutOff=4)
    ...
    for pairConfigName in paramsDict['Pair Configurations']:
        ...
        svmModel = svmutil.svm_load_model(os.pardir + pairConfig['Model'])
        svmRange = svmutil.load_ranges(os.pardir + os.path.splitext(pairConfig['Model'])[0] + '.range')
