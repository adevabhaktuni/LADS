...
def normalize_instances(instances, ranges = None, lower=-1., upper=1.):
    normalized_instances = copy.deepcopy(instances)
    if ranges == None:
        ranges_dict = dict()
	ranges_dict['lower'] = lower
	ranges_dict['upper'] = upper
    else:
        lower = ranges['lower']
        upper = ranges['upper']
...
def load_ranges(ranges_file):
    ranges_in = open(ranges_file)
    ranges_dict = {}
    var = ranges_in.readline()
    lower, upper = ranges_in.readline().split(' ')
    ranges_dict['lower'] = float(lower)
    ranges_dict['upper'] = float(upper)
    
    for line in ranges_in:
       attr, minimum, maximum = line.split(' ') 
       ranges_dict[int(attr)] = [float(minimum), float(maximum)]
    
    return ranges_dict

def save_ranges(ranges_dict, ranges_file):
    ranges_out = open(ranges_file, 'w')
    ranges_out.write('x\n')
    ranges_out.write('%f %f\n' % (ranges_dict['lower'], ranges_dict['upper']))
    for attr in sorted(ranges_dict.keys()):
        try:
            ranges_out.write('%i %f %f\n' % (attr, ranges_dict[attr][0], ranges_dict[attr][1]))
	except TypeError:
	    pass	
